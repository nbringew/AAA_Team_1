...
% Apply the effect of \noindent consistently to all report paragraphs.
\setlength{\parindent}{0pt}

% Allow the [H] float specifier (place floats exactly where written); used in
% the appendix to keep its many figures/tables in source order.
\usepackage{float}

% Each front-matter list starts on its own page.
\usepackage{etoolbox}
\pretocmd{\listoffigures}{\clearpage}{}{}
\pretocmd{\listoftables}{\clearpage}{}{}

% List of Equations. LaTeX has no built-in list for equations, so this defines
% one backed by a .loe file: \eqentry{...} placed after a numbered equation
% records it, and \listofequations typesets the collected entries. Entries reuse
% \l@figure so they are formatted like the list of figures and tables.
\makeatletter
\newcommand{\listofequations}{%
    \clearpage
    \section*{List of Equations}%
    \@mkboth{List of Equations}{List of Equations}%
    \@starttoc{loe}%
}
\newcommand{\eqentry}[1]{%
